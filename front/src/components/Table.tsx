...
export const Table: React.FC<ITableProps> = ({ head, values, onClick, getInfo, icon, title }) => {
    const [isLoading, setLoading] = useState<boolean>(false);
    const currentValues = values.slice(0, 5);
            ...
            <div className="flex flex-col">
                <div className="py-2 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                    <section className="inline-block overflow-x-auto align-middle shadow-md dark:shadow-white rounded-lg w-full bg-white-light dark:bg-primary-light pb-3 dark:text-white">
                        ...
                        {isLoading ? (
                            <article className="flex justify-center h-80 items-center">
                                <svg role="status" className="inline mr-2 w-7 h-7 animate-spin fill-blue text-brown" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <table className="w-full border-t border-t-brown">
                                <thead>
                                    <tr>
                                    {head.map((title, i) => {
                                        return (
                                ...
                                <tbody className="bg-white-light dark:bg-primary-light border-t border-t-brown">
                                    {Array.from({ length: currentValues.length }, (_, i) =>
                                        <TableRow head={head} values={currentValues[i]} key={i} index={i+1}/>
                                    )}
                                </tbody>
                            </table>
                        )}
