...
export const Dropdown: React.FC<IDropdownProps> = ({ title, color, values, display, setDisplay}) => {
    ...
    return (
        <div className="group inline-block mx-2">
            ...
            <button className="outline-none focus:outline-none border dark:border-white px-3 py-2 bg-white dark:bg-primary-light dark:text-white rounded-md flex items-center w-72">
                <span className={`pr-1 font-sm flex-1 ${display === 'pricing' && 'text-brown'}`}>{display.toUpperCase()}</span>
                <span>
                    <svg
                        className="fill-current dark:fill-white h-4 w-4 transform group-hover:-rotate-180
            ...
            <ul className="bg-white dark:bg-primary-light border-b border-r border-l rounded-bl-lg rounded-br-lg transform scale-0 group-hover:scale-100 absolute w-72 transition duration-150 ease-in-out origin-top">
                {values.map((value, index) => (
                    <li key={index} className={`rounded-sm px-3 py-1 hover:${color} hover:bg-opacity-60 text-center dark:text-white`} onClick={() => {setDisplay(value)}}>
